import { getIcalTextFromEvents, getIcalObjectFromText, convertToDate, } from "../../IcalUtils";
import { DEFAULT_TZID, EVENT_ALL_VALUES_DEFAULT_TZID, EVENT_ALL_VALUES_NON_DEFAULT_TZID, EVENT_ALL_VALUES_NO_TZID, EVENT_REQUIRED_VALUES_NO_TZID, NON_DEFAULT_CALENDAR_TZID, } from "./IcalTestConstants";
// import { getVtimezoneComponent } from "@touch4it/ical-timezones";
// import { AggEvent } from "src/models/AggEvent";
export function verifyEventsFromInputArray(inputArray) {
 ...
 * @param secondObject
 */
export function expectObjectToBeSimilar(baseObject, secondObject) {
    for (const key of Object.keys(baseObject)) {
        expect(`${key}: ${secondObject[key]}`).toEqual(`${key}: ${baseObject[key]}`);
    }
}
export function verifyEventFromInput(inputEvent) {
...
function getExpected(inputEvent) {
